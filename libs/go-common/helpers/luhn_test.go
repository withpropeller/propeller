...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{"8", false},
		{"0", true},
		{"50610001001035198", false},
	}
	for _, test := range tests {
		t.Run(test.number, func(t *testing.T) {
			res := ValidateLuhn(test.number)
			if res != nil {
				if test.expectOK {
...
func TestCalculate(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.expected, func(t *testing.T) {
			l, n, err := CalculateLuhn(test.number)
			if err != nil {
	...
	for _, test := range tests {
		t.Run(strconv.Itoa(test.numberSize), func(t *testing.T) {
			for i := 0; i < test.sampleSize; i++ {
				err := ValidateLuhn(GenerateLuhn(test.numberSize))
				if err != nil {
					log.Printf("Unexpected err %+v", err)
					t.Fail()
				}
...
func TestGenerateWithPrefix(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(strconv.Itoa(test.numberSize), func(t *testing.T) {
			for i := 0; i < test.sampleSize; i++ {
				err := ValidateLuhn(GenerateWithPrefix("123", test.numberSize))
				if err != nil {
					log.Printf("Unexpected err %+v", err)
					t.Fail()
				}
