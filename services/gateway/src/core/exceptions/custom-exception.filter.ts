...
export class CustomExceptionFilter implements ExceptionFilter {
    constructor(private readonly config: ConfigService, private reporter: ReporterService, private jwt: JwtService) {}
    ...
    public catch(exception: CustomException, host: ArgumentsHost) {
        ...
        this.reporter.pushError({
            error: data,
            requestData: Utils.pickKeys(requestData, '-password -oldPassword -newPassword -stateToken -passcode -token'),
            requestUrl: request.url,
            requestUser: jwt ? jwt['email'] : null,
        });
        response.status(exception.getStatus()).json({
            ...data,
            error: this.config.inProduction ? undefined : exception.getError(),
        });
    }
}
