...
@ValidatorConstraint({ name: 'cardLimitAmount', async: false })
export class CardLimitAmount implements ValidatorConstraintInterface {
    validate(amount: number, args: ValidationArguments) {
        return this.validateAmount(amount, args.object['currency']);
    }

    validateAmount(amount: number, currency: TransactionCurrency) {
        return currency === TransactionCurrency.USD ? amount >= 500 : amount >= 100000;
    }

    defaultMessage(validationArguments: ValidationArguments) {
        return `${validationArguments.property} must be a valid amount`;
    }
}
...
@ValidatorConstraint({ name: 'mongoId', async: false })
export class TagId implements ValidatorConstraintInterface {
    validate(id: string, args: ValidationArguments) {
...
@ValidatorConstraint({ name: 'tagId', async: false })
export class TagIV2 implements ValidatorConstraintInterface {
    validate(str: string, args: ValidationArguments) {

        const id = Utils.parseIdTag(str);
