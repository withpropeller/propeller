...
import { FloService, floDecodeJson, floEncode } from '@core/services/flo.service';
...
export class FxService {
    ...
    async getFxRate(from: TransactionCurrency, to: TransactionCurrency): Promise<FxRate> {
        ...
        try {
            const record = floDecodeJson<{ timestamp: number; value: number }>(
                (await this.flo.client.kv.jsonGet(key, '$'))?.value ?? null,
            );
            if (!record) throw new Error('not found');
            const exchangeRate = Number(record.value);
