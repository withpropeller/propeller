...
const DEFAULT_EXCLUDE_EXPAND = ['business', 'id'];
const FILTER_DATE_PATHS = ['txTime', 'createdAt', 'updatedAt', 'date'];
...
export class MongoAPIPaging {
    ...
    static getPagingConstraints(
        ...
        return { select, conditions, projections, limit, sort, populate, reverseList };
    }
    ...
    static parseProjection(projection: string | string[]) {
        const projectionArr = Array.isArray(projection) ? projection : [projection];

        return projectionArr.reduce((prev, curr) => {
            try {
    ...
    static parseSort(sort: string, options: MongoAPIPagingOptions): Sort {
        if (!sort) {
            return options?.defaultSort ?? this.SORT_DEFAULT;
        }
        ...
        const newEntries = entries.map(([key, value]) => {
            if (FILTER_MODEL_PATHS.includes(key)) {
                if (Utils.isObject(value)) {
                    ...
                    return [key, Object.fromEntries(newEntries)];
                }
    ...
    private static genLessThanWhere(entry: WhereEntry) {
        if (FILTER_DATE_PATHS.includes(entry.field)) {
            return this.genLessThanDateWhere(entry);
        }
    ...
    private static genLessThanDateWhere(entry: WhereEntry) {
        return { $lt: new Date(entry.value) };
    }
    ...
    private static genLessThanOrEqualDateWhere(entry: WhereEntry) {
        return { $lte: new Date(entry.value) };
    }
    ...
    private static genGreaterThanDateWhere(entry: WhereEntry) {
        return { $gt: new Date(entry.value) };
    }
    ...
    private static genGreaterThanOrEqualDateWhere(entry: WhereEntry) {
        return { $gte: new Date(entry.value) };
    }
