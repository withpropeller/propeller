...
import { MultiFactorSettings } from '@api/users';
...
export class MultiFactorAuth {
    constructor(private adminService: AdminService, private notificationHandler: NotificationHandler) {}

    async getMFASettings(publicId: Types.ObjectId) {
        const user = await this.adminService.findOneAndUpdate({ _id: publicId }, { $set: { stateToken: null } });
        ...
        switch (data.channel) {
            case TwoFAChannels.Phone:
                options = {
                    ...
                    messagingChannel: data.messagingChannel || MessagingChannel.SMS,
                };
                break;
    ...
    async deactivate2FA(publicId: Types.ObjectId, data: Setup2FADto, sendToken = false) {
        ...
        return this.adminService.updateById(user.id, {
            $set: { multiFactors },
        });
    }
    ...
    addMFA(user: HydratedDocument<Admin>, mfa: MultiFactorSettings) {
    ...
    verifyTOTPSecret(user: Admin, channel: TwoFAChannels, token: string, checkEnabled = true) {
        const multiFactors = user.multiFactors || [];
        const factor = multiFactors.find((v) => v.channel === channel);
    ...
    send2FAToken(user: Admin, mfa: MultiFactorSettings) {
     ...
     * @param user
     */
    async handle2FATokenEmail(user: Admin, token: string): Promise<void> {
