...
export class PaymentException extends CustomException {
    ...
    static get PaymentNotInFailedState() {
        return new this('Payment not in failed state', PaymentErrors.PaymentNotInFailedState);
    }

    static get PaymentNotRefundable() {
        return new this('Payment not refundable', PaymentErrors.PaymentNotRefundable);
    }

    static get PaymentNotInSuccessState() {
        return new this('Payment not in success state', PaymentErrors.PaymentNotInSuccessState);
    }
    ...
    static get CannotValidatePayment() {
        return new this('Cannot validate the type/method of payment', PaymentErrors.CannotValidatePayment);
    }

    static get CannotForceChargePayment() {
        return new this('Cannot force charge the payment', PaymentErrors.CannotForceChargePayment);
    }

    static get MethodNotSupported() {
        return new this('Payment method not supported', PaymentErrors.MethodNotSupported);
    }
}
