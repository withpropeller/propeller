...
@Schema(
    ModelSchemaOptions({
        objectIds: ['business', 'roles:rol', 'kyc'],
        pick: '-passwordHash -multiFactors -stateToken',
        tag: ModelIdTag.Admin,
    }),
)
export class Admin {
...
AdminSchema.virtual('fullName').get(function (this: HydratedDocument<Admin>) {
    return this.firstName && this.lastName ? `${this.firstName} ${this.lastName}` : undefined;
});

AdminSchema.virtual('multiFactorEnabled').get(function (this: HydratedDocument<Admin>) {
