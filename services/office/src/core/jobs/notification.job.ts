...
export function ExtractNotificationTo(
    user: User | Admin,
    allowedChannels = [NotificationChannel.Email, NotificationChannel.Push],
) {
    ...
    if (allowedChannels.includes(NotificationChannel.Push) && user.integration?.onesignal) {
        to = {
            ...to,
            push: { id: user.integration.onesignal.id, name: user.fullName },
        };
    }
...
export function ExtractEmailNotificationTo(user: User | Admin) {
...
export function ExtractAllNotificationTo(user: User) {
    return ExtractNotificationTo(user, [
