...
export abstract class Repository<T> {
    ...
    async deleteById(publicId: string, options?: ExecutionOptions) {
        if (options?.dryRun) {
            return;
        }
    ...
    bulkInsert(docs: Partial<T>[], options?: InsertManyOptions & ExecutionOptions): Promise<HydratedDocument<T>[]> {
